/*
  # Complete RBAC System with Salon Management

  1. Database Schema Updates
    - Add role column to users table
    - Create salons, services, bookings, reviews, and salon_staff tables
...
-- First, add the role column to users table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.users ADD COLUMN role text DEFAULT 'user' CHECK (role IN ('user', 'salon_owner', 'admin'));
  END IF;
END $$;
...
-- Create updated_at triggers for all tables (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
